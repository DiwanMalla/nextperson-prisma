...
const HomePage: React.FC = () => {
  ...
  const handleOpen = (person: Person | null) => {
    if (person) {
      // If person is not null, ensure dob is a Date object
      setCurrentPerson({
        ...person,
        dob: person.dob ? new Date(person.dob) : null, // Convert to Date
      });
    } else {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editMode && currentPerson) {
        ...
        // Exclude id from the new person object if it exists
        const { id, ...newPersonData } = currentPerson || {};
